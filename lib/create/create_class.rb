	...
	module Create
		class Class
			def self.run_interactive
				...
				exit 0 if class_name.nil? || class_name.length.zero?

				class_path = Ask.input 'Enter the destination path'
				if class_path.nil? || class_path.length.zero?
			...
			def self.run(class_name, class_path)
				...
				class_destination = class_path.end_with?('.swift') ? class_path : File.join(class_path, "#{class_name}.swift")
