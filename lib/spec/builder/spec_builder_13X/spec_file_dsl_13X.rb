require_relative 'spec_configuration_dsl_13X'
require_relative 'spec_target_dsl_13X'
require_relative 'spec_variant_dsl_13X'

module StructCore
	class SpecFileDSL13X
		def initialize
			@spec_file = nil
		...
		def configuration(name = nil, &block)
			...
			dsl = StructCore::SpecConfigurationDSL13X.new
		...
		def variant(name = nil, abstract = false, &block)
			...
			dsl = StructCore::SpecVariantDSL13X.new
			dsl.project_configurations = @spec_file.configurations
			dsl.project_base_dir = @project_base_dir
