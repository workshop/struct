...
module StructCore
	class Specparser30X
		...
		def parse(spec_version, spec_hash, filename)
			...
			pre_generate_script, post_generate_script, other_scripts = parse_scripts spec_hash['scripts'] || {}, project_base_dir
			schemes = parse_schemes spec_hash['schemes']

			Specfile.new(spec_version, targets, configurations, variants, project_base_dir, @spec_file_uses_pods, pre_generate_script, post_generate_script, schemes, other_scripts)
		end
		...
		def parse_scripts(scripts_opts, project_base_dir)
			...
			other_scripts = scripts_opts.select { |key, value|
				!%w(pre-generate post-generate).include? key
			}.map { |key, value|
				[key, StructCore::Specfile::HookScript.new(File.join(project_base_dir, value))]
			}.to_h

			[pre_generate, post_generate, other_scripts]
		end
