...
RSpec.describe StructCore::Specparser12X do
	describe '#can_parse_version' do
		it 'specifies that it can only parse Spec versions 1.2.X' do
			...
			expect(parser.can_parse_version(StructCore::SPEC_VERSION_120)).to be_truthy
			expect(parser.can_parse_version(StructCore::SPEC_VERSION_121)).to be_truthy
		...
		describe '#parse' do
			it 'can parse a specfile with only configurations' do
				...
				proj = parser.parse StructCore::SPEC_VERSION_120, test_hash, project_file
			...
			it 'raises an error if a project doesn\'t contain configurations' do
				...
				expect { parser.parse StructCore::SPEC_VERSION_120, test_hash, project_file }.to raise_error(StandardError)
			end
