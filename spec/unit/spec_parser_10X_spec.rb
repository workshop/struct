...
RSpec.describe StructCore::Specparser11X do
	describe '#can_parse_version' do
		it 'specifies that it can parse Spec versions 1.0.X' do
			parser = StructCore::Specparser11X.new

			expect(parser.can_parse_version(StructCore::SPEC_VERSION_100)).to be_truthy
	...
	describe '#parse' do
		it 'can parse a specfile with only configurations' do
			...
			proj = parser.parse StructCore::SPEC_VERSION_100, test_hash, project_file
		...
		it 'raises an error if a project doesn\'t contain configurations' do
			...
			expect { parser.parse StructCore::SPEC_VERSION_100, test_hash, project_file }.to raise_error(StandardError)
		end
