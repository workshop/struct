...
RSpec.describe StructCore::SpecProcessor do
	describe '#process' do
		it 'can write a working project with cross platform targets' do
			next if should_stub_tests_on_incompatible_os
			destination = Dir.mktmpdir
			copy_support_files File.join(File.dirname(__FILE__), 'support_files', 'xplat_test'), destination
			...
			Dir.chdir(File.join(File.dirname(__FILE__), 'support_files')) {
				expect { Fastlane::LaneManager.cruise_lane(nil, 'buildios', {:project => project_file}, {}) }.to_not raise_error
			}
